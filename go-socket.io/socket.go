...
func (s *socket) Close() error {
	//s.conn.Close()
	//fmt.Println(">>>>>>>>>>>>>>>>>>>socket call close!!  emit disconnect")
	//debug.PrintStack()
	s.Emit("disconnect")
	return nil
}

func (s *socket) RoomMemCount(room string) int {
	return s.socketHandler.RoomMemCount(room)
}
...
func (s *socket) loop() error {
	defer func() {
		//fmt.Println(">>>>>>>>>>>>>>>>>>>loop finish call func  Disconnet")
		s.LeaveAll()
		p := packet{
			Type: _DISCONNECT,
			Id:   -1,
	...
	s.socketHandler.onPacket(nil, &p)
	//	fmt.Println(">>>>>>>>>>>>>>>>>>>decoder.Decode for Begin")
	for {
		//	fmt.Println(">>>>>>>>>>>>>>>>>>>decoder.Decode new")
		decoder := newDecoder(s.conn)
		var p packet
		//fmt.Println(">>>>>>>>>>>>>>>>>>>socket loop recv msg:", p.Type)
		if err := decoder.Decode(&p); err != nil {
			//fmt.Println(">>>>>>>>>>>>>>>>>>>decoder 159 err:", err)
			return err
		}
