...
//use param to connect
func SetConfigParam(username, password, hostname, port, database, charset string) (*Model, error) {
	c := new(Model)
	db, err := sql.Open("mysql", username+":"+password+"@tcp("+hostname+":"+port+")/"+database+"?charset="+charset)
...
func (m *Model) Insert(param map[string]interface{}) (num int64, err error) {
	if m.db == nil {
	...
	if err != nil {
		...
		err = errors.New(fmt.Sprint("inster sql failure,", err.Error()))
		return 0, err
	}
...
func (m *Model) Update(param map[string]interface{}) (num int64, err error) {
	if m.db == nil {
		return 0, errors.New("mysql not connect")
	}
	var setValue []string
	for key, value := range param {
		...
		case int64:
			set := fmt.Sprintf("%v = %v", key, value.(int64))
			setValue = append(setValue, set)
		case string:
