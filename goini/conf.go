...
func (c *Config) GetValueString(section, name, defaultString string) string {
	c.ReadList()
	conf := c.ReadList()
	for _, v := range conf {
		for key, value := range v {
...
func (c *Config) GetValueInt(section, name string, defaultInt int) int {
	c.ReadList()
	conf := c.ReadList()
	for _, v := range conf {
		for key, value := range v {
			if key == section {
				i, _ := strconv.Atoi(value[name])
				return i
			}
		}
	}
